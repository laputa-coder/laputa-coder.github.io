...
- 评价算法的好坏：渐近时间复杂度和渐近空间复杂度。
  ...
  ![](/assets/assets//assets/images/img_HandsOnML/img_ML/img_Py/algorithm_complexity_1.png)

  ![](/assets/assets//assets/images/img_HandsOnML/img_ML/img_Py/algorithm_complexity_2.png)
...
### 装饰器函数（使用装饰器和取消装饰器）
  ...
  def record(/assets/images/img_HandsOnML/output):
      """可以参数化的装饰器"""
  	...
  	def decorate(func):
  		...
  		def wrapper(*args, **kwargs):
  			start = time()
  			result = func(*args, **kwargs)
  			/assets/images/img_HandsOnML/output(func.__name__, time() - start)
  			return result
  ...
  class Record():
      ...
      def __init__(self, /assets/images/img_HandsOnML/output):
          self./assets/images/img_HandsOnML/output = /assets/images/img_HandsOnML/output
      ...
      def __call__(self, func):
          ...
          def wrapper(*args, **kwargs):
              start = time()
              result = func(*args, **kwargs)
              self./assets/images/img_HandsOnML/output(func.__name__, time() - start)
              return result
...
- 多线程：python中提供了`Thread`类并辅以`Lock`、`Condition`、`Event`、`Semaphore`和`Barrier`。python中有GIL来防止多个线程同时执行本地字节码，这个锁对于Cpython是必须的，因为Cpython的内存管理并不是线程安全的，因为GIL的存在多线程并不能发挥CPU的多核特性。
  ...
  def main():
  	...
  	if not os.path.exists(PREFIX):
  		os.mkdir(PREFIX)
  	for infile in glob.glob('assets//assets/images/img_HandsOnML/img_ML/*.png'):
